...
@export var curve_points: PackedVector3Array : set = set_points, get = get_points
@export var width: float = 0.1 : set = set_width
...
@export_enum("None", "Tile", "Stretch", "Double Sided Tile") var texture_mode: int = 2 : set = set_texture_mode

@export var flat : bool = false : set = set_flat
@export_enum("Follow Main Camera", "Custom") var flat_direction: int = 0 : set = set_flat_direction
@export var custom_flat_direction := Vector3(0,1,0) : set = set_custom_flat_direction
@export var resolution: float = 1.0 : set = set_resolution
@export var cross_section_resolution: int = 10 : set = set_cross_section_resolution
@export var smooth: bool = false : set = set_smooth

@export var generate_collision_mesh := false : set = set_generate_collision_mesh
@export_enum("Convex", "Concave") var collision_mesh_type: int

@export var global_coords : bool = false : set = set_global_coords

@export var custom_material: Material : set = set_material
...
func _ready() -> void:
	connect('curve_changed', Callable(self, 'update'))
#	if Engine.editor_hint:
...
func set_width_curve(v):
	width_curve = v
	if width_curve != null:
		width_curve.connect('changed', Callable(self, 'update'))
	update()
...
func set_gradient(v):
	gradient = v
	if gradient != null:
		gradient.connect('changed', Callable(self, 'update'))
	update()
